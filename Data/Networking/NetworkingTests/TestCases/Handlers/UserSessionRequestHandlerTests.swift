...
class UserSessionRequestHandlerTests: XCTestCase {
    ...
    private var errorFilterSpy: ErrorFilterSpy!
    private var oauthHandlerSpy: OAuthHandlerSpy!

    override func setUp() {
        super.setUp()

        errorFilterSpy = ErrorFilterSpy()
        oauthHandlerSpy = OAuthHandlerSpy()
        sut = UserSessionRequestHandler(environment: environmentStub,
                                        errorFilter: errorFilterSpy, oauthHandler: oauthHandlerSpy)
    }
...
//    }
    ...
    func testAdaptURLReques() {
        ...
        sut.adapt(urlRequest, for: sessionStub) { result in
            ...
            XCTAssert(self.oauthHandlerSpy.authorizeRequestCalled)
        }
    }
